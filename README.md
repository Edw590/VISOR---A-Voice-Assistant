...
### - To use the project
- Download this main project and the Advanced Commands Detection module (`git clone --recursive [repo link here]`). Then go to the `ClientCode` folder and run the command `go build -tags=client .`. For the server, go to the `ServerCode` folder and run the command `go build -tags=server .` (to completely use the server program, open port 3234 on your router so that the clients can communicate with the server and vice-versa). Finally move both files to the `bin` folder.
