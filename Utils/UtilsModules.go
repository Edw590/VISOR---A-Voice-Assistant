...
type ModuleInfo[T any] struct {
	...
	Name string
	// Number is the number of the module.
	Number int
...
*/
func ModStartup[T any](mod_num int, realMain RealMain, module *Module) {
...
func (moduleInfo *ModuleInfo[T]) UpdateGenInfo() error {
	...
	var file_path_curr GPath = GetUserDataDirMODULES(moduleInfo.Number).Add2(false, _MOD_GEN_INFO_JSON)
	var file_path_new GPath = GetUserDataDirMODULES(moduleInfo.Number).Add2(false, _MOD_GEN_INFO_JSON_TMP)
...
func printShutdownSequenceMODULES(errors bool, mod_name string, mod_num string) {
	...
	log.Println("---------")
	if errors {
		log.Println("Exiting with ERRORS the module \"" + mod_name + "\" (number " + mod_num + ")...")
	} else {
		log.Println("Exiting normally the module \"" + mod_name + "\" (number " + mod_num + ")...")
	}
	log.Println("\\\\------------------------------------------//")
}
...
func (moduleInfo *ModuleInfo[T]) updateModRunInfo() {
	var mod_num int = moduleInfo.Number
